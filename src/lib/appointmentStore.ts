...
export const appointmentStore = {
  ...
  async getAppointmentsByBarber(barberName: string, date: string): Promise<string[]> {
    ...
    // Calculate all blocked time slots based on appointment duration
    ...
    data.forEach(appointment => {
      ...
      const duration = appointment.duration || 60; // Default to 60 minutes if not specified
      const startIndex = allTimeSlots.indexOf(startTime);
      
      if (startIndex !== -1) {
        // Each slot is 15 minutes, so calculate how many slots to block
